...
import csv
# import re module for regular expression functionality
import re
...
def recursive(path):
    ...
    for root, dirs, files in os.walk(path):
        for index in files:
            temp_array.append(os.path.join(root, index))
    temp_array = re.sub(r",","",str(temp_array))
    temp_array = re.sub(r" ","\n",temp_array)
    temp_array = re.sub(r"[\[\]]","",temp_array)
    global_array = temp_array.split("\n")
    print(global_array)        
    #print(global_array[1])
    #print(global_array[2])
    #if type(global_array) is str:
    #    print("global_array is a string")
    #elif type(global_array) is list:
    #    print("global_array is list")
    #else print("neither")
...
def main():
    ...
    # ---------------------------
    # populate(global_array[3]) # Change the index for the current file you're trying to view metadata for. I haven't made it automated with a loop yet.
